...
export const GlossaryTranslations = ({ glossary, lang, usedLangs, canEdit, saveTranslations }: IProps) => {
  ...
  const headerControls = [<button key="settings" onClick={handleShowLanguageSettings} style={{marginRight: 10}}>Language Settings</button>]
  if (canEdit) {
    headerControls.push(<button key="delete" onClick={handleDeleteLanguage}>Delete Language</button>)
  }
